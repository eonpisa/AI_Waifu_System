...
if __name__ == "__main__":
    ...
    async def voice(
        ...
        model_file: Optional[str] = Query(
            None,
            description="モデルファイル名またはパス。指定時はその .safetensors を使用",
        ),
        model_id: int = Query(
        ...
        # Keep the original first-file behavior unless a client explicitly
        # selects a checkpoint.
        if model_file:
            selected_model_name = model_name or model_holder.model_names[model_id]
            selected_model_path = Path(model_file)
            if not selected_model_path.is_absolute():
                selected_model_path = (
                    model_holder.root_dir / selected_model_name / selected_model_path
                )
            try:
                model = model_holder.get_model(
                    selected_model_name, str(selected_model_path)
                )
            except ValueError as e:
                raise_validation_error(str(e), "model_file")
        else:
