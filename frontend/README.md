...
브라우저는 TTS WAV를 재생하지 않는다. 음성 입력 버튼을 누르면 마이크 권한을 요청한다.

기존 실제 웹 4턴의 문맥·Gemini 자막·음성·립싱크는 사용자 검수를 통과했다.
새 서비스 상태·번역기 표시는 자동 검증을 완료했다. Mac의 웹 대화에서
Gemini 채택·폴백 없음과 음성·입 움직임·입력 복귀를 사용자 확인했다.
...
입력창이 비어 있을 때 마이크 버튼으로 녹음을 시작하고, 다시 누르면 종료한다.
최대 20초이며 인식한 한국어를 입력창에서 수정한 뒤 직접 전송한다.
녹음 취소·마이크 권한 거부·전사 실패 뒤에는 키보드 입력을 사용할 수 있다.
대화 처리 중에는 녹음하지 않으며, 미리보기에서는 마이크가 비활성화돼 있다.
음성은 로컬 API의 `/api/transcribe`에만 보내고 브라우저 저장소에 보관하지 않는다.
STT 모델 설정은 [백엔드 안내](../docs/backend-api.md#선택형-음성-입력)를 참고한다.
Mac에서 마이크 전사·정상 대화와 녹음 취소 후 키보드 입력 복귀를 확인했다.
Windows 브라우저·마이크 실환경 검수는 아직 진행하지 않았다.
...
| `src/useMicrophone.ts` | 마이크 권한·녹음·전사 초안·취소와 장치 해제 |
...
자동 검증은 화면·연결·STT 오류 처리를 모의 마이크와 대역 서비스로 검사한다.
기존 HTTP/WS 상태 일치, 폴백·복구, CORS·접수 경합·종료, IME 검사도 유지한다.
실제 Mac 웹 대화에서는 마이크 전사, Gemini 자막, SBV2 음성,
VTS 입 움직임과 다음 입력 복귀를 사용자 확인했다. Windows 검수는 남아 있다.
