...
class TranslatorTests(unittest.TestCase):
    ...
    def test_japanese_input_skips_translation(self):
        with patch("backend.translation.translator.chat") as chat_mock:
    ...
    def test_typo_is_normalized_before_japanese_translation(self):
        ...
        with patch("backend.translation.translator.chat", return_value=response):
    ...
    def test_normal_greeting_rewritten_to_a_different_tone_is_rejected(self):
        ...
        with patch("backend.translation.translator.chat", side_effect=[invalid, invalid]):
    ...
    def test_ambiguous_input_is_not_translated(self):
        ...
        with patch("backend.translation.translator.chat", return_value=response) as chat_mock:
    ...
    def test_korean_to_japanese_retries_note_and_parenthetical_output(self):
        ...
        with patch("backend.translation.translator.chat", side_effect=[invalid, valid]) as chat_mock:
    ...
    def test_subtitle_retries_japanese_and_note_mixed_output(self):
        ...
        with patch("backend.translation.translator.chat", side_effect=outputs) as chat_mock:
    ...
    def test_japanese_to_korean_uses_gemini_without_calling_qwen(self):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "secret-key"}, clear=False):
            with patch("backend.translation.gemini_translator.requests.post", return_value=response) as post:
                with patch("backend.translation.translator.chat") as chat_mock:
    ...
    def test_invalid_gemini_translation_falls_back_to_existing_qwen_path(self):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "secret-key"}, clear=False):
            with patch("backend.translation.gemini_translator.requests.post", return_value=response):
                with patch("backend.translation.translator.chat", return_value=qwen_output) as chat_mock:
    ...
    def test_gemini_request_failure_falls_back_to_existing_qwen_path(self):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "secret-key"}, clear=False):
            with patch(
                "backend.translation.gemini_translator.requests.post",
    ...
    def test_missing_gemini_key_skips_api_and_uses_qwen(self):
        with patch("backend.translation.gemini_translator.requests.post") as post:
            with patch(
                "backend.translation.translator.chat", return_value=json_translation("안녕하세요")
            ) as chat_mock:
                result = translator.japanese_to_korean("こんにちは")
    ...
    def test_subtitle_rejects_truncated_long_korean_output(self):
        ...
        with patch("backend.translation.translator.chat", side_effect=outputs):
    ...
    def test_subtitle_uses_source_after_two_invalid_results(self):
        with patch("backend.translation.translator.chat", side_effect=[json_translation("你好。"), json_translation("こんにちは")]):
    ...
    def test_mixed_japanese_korean_subtitle_is_regenerated_from_full_source(self):
        ...
        with patch("backend.translation.translator.chat", side_effect=[json_translation(mixed), json_translation(complete)]) as chat_mock:
    ...
    def test_japanese_to_korean_requests_are_isolated_and_log_only_adopted_provider(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("backend.translation.translator.chat", side_effect=[json_translation(mixed), json_translation(valid)]) as chat_mock:
    ...
    def test_dedicated_japanese_to_korean_model_failure_does_not_switch_models(self):
        with patch.dict(os.environ, {"OLLAMA_JA_TO_KO_MODEL": "missing-subtitle-model"}, clear=False):
            with patch("backend.translation.translator.chat", side_effect=ConnectionError("model unavailable")) as chat_mock:
                self.assertIsNone(translator.japanese_to_korean("こんにちは"))
        self.assertEqual(chat_mock.call_count, 2)
        self.assertEqual(
    ...
    def test_two_mixed_language_subtitles_fail_without_japanese_fallback(self):
        ...
        with patch("backend.translation.translator.chat", side_effect=[json_translation(mixed), json_translation(mixed)]):
    ...
    def test_korean_input_timeout_is_distinct_and_retries_once(self):
        ...
        with patch("backend.translation.translator.chat", side_effect=[ReadTimeout("read timeout"), valid]) as chat_mock:
    ...
    def test_gemini_success_logs_only_validated_provider_and_model(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("backend.translation.gemini_translator.requests.post", return_value=gemini_response(subtitle)):
                with patch("backend.translation.translator.chat") as chat_mock, self.assertLogs(level="DEBUG") as logs:
    ...
    def test_rejected_gemini_is_never_logged_as_adopted(self):
        with patch.dict(os.environ, {"GEMINI_API_KEY": "test-only-secret", "AI_WAIFU_DEBUG": "1"}, clear=False):
            with patch("backend.translation.gemini_translator.requests.post", return_value=gemini_response("privateword 안녕하세요")):
                with patch("backend.translation.translator.chat", return_value=json_translation("안녕하세요")):
    ...
    def test_gemini_exceptions_log_codes_without_key_headers_or_exception_text(self):
        ...
        for error, reason in cases:
            with self.subTest(reason=reason), patch.dict(os.environ, {"GEMINI_API_KEY": secret, "AI_WAIFU_DEBUG": "1"}, clear=False):
                with patch("backend.translation.gemini_translator.requests.post", side_effect=error):
                    with patch("backend.translation.translator.chat", return_value=json_translation("안녕하세요")) as chat_mock:
                        with self.assertLogs(level="DEBUG") as logs:
                            self.assertEqual(translator.japanese_to_korean("こんにちは"), "안녕하세요")
                self.assertEqual(chat_mock.call_count, 1)
                self.assertEqual(len(logs.records), 1)
    ...
    def test_qwen_failure_does_not_claim_a_provider_was_adopted(self):
        with patch.dict(os.environ, {"AI_WAIFU_DEBUG": "1"}, clear=False):
            with patch("backend.translation.translator.chat", side_effect=RequestException("private headers and response")) as chat_mock:
                with self.assertLogs(level="DEBUG") as logs:
                    self.assertIsNone(translator.japanese_to_korean("こんにちは"))
        self.assertEqual(chat_mock.call_count, 2)
        self.assertEqual(len(logs.records), 1)
    ...
    def test_korean_input_errors_do_not_log_exception_contents(self):
        ...
        for error in (ReadTimeout(sensitive), RequestException(sensitive)):
            with self.subTest(error=type(error).__name__), patch("backend.translation.translator.chat", side_effect=error):
                with self.assertLogs("translator", level="DEBUG") as logs:
                    self.assertIsNone(translator.translate_korean_input("안녕"))
    ...
    def test_missing_japanese_sentence_still_retries(self):
        ...
        with patch("backend.translation.translator.chat", side_effect=responses) as chat_mock:
            result = translator.translate_korean_input(source)
        self.assertIsNotNone(result)
